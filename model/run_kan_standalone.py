"""
run_kan_standalone.py
----------------------
Standalone script to train, evaluate, predict, and explain using the Standalone KAN Model
(KAN Feature Embedding + MLP Classifier Head without Transformer Self-Attention).
"""
...
# Define Standalone KAN Classifier
class SimpleKANClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        tokens = self.kan_embedding(x)
        return self.classifier(tokens)

def main():
    print("=" * 70)
    print(" 🎯 STANDALONE KAN MODEL: TRAINING, PREDICTION & EXPLAINABILITY")
    print("=" * 70)
    ...
    # 1. Train Standalone KAN
    print("\n🚀 Step 1: Training Standalone KAN Model ...")
    model = SimpleKANClassifier(num_features, num_classes, embed_dim=32, num_grids=8).to(DEVICE)
    ...
    print("\n📊 Standalone KAN Performance Metrics:")
    ...
    # 3. Sample Prediction
    print("\n🔮 Step 2: Running Sample Prediction with Standalone KAN ...")
    ...
    # 4. SHAP Explanation
    print("\n🧠 Step 3: Computing SHAP Feature Attribution for Standalone KAN ...")
