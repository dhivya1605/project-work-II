...
from samplers.truncated_normal import truncated_normal_sample
from utils.categorical_utils import choose_random

random.seed(RANDOM_SEED)

def generate_truncated_normal_dataset(df):
    ...
    for _, row in df.iterrows():
        ...
        soil_ph = truncated_normal_sample(
            row["SOIL_PH_LOW"],
            row["SOIL_PH_HIGH"],
            SAMPLES_PER_CROP
        )

        duration = truncated_normal_sample(
            row["CROPDURATION_MIN"],
            row["CROPDURATION_MAX"],
            SAMPLES_PER_CROP
        )

        temperature = truncated_normal_sample(
            row["MIN_TEMP"],
            row["MAX_TEMP"],
            SAMPLES_PER_CROP
        )

        water = truncated_normal_sample(
            row["WATERREQUIRED_MIN"],
            row["WATERREQUIRED_MAX"],
            SAMPLES_PER_CROP
        )

        humidity = truncated_normal_sample(
            row["RELATIVE_HUMIDITY_MIN"],
            row["RELATIVE_HUMIDITY_MAX"],
            SAMPLES_PER_CROP
        )

        nitrogen = truncated_normal_sample(
            row["N_MIN"],
            row["N_MAX"],
            SAMPLES_PER_CROP
        )

        phosphorus = truncated_normal_sample(
            row["P_MIN"],
            row["P_MAX"],
            SAMPLES_PER_CROP
        )

        potassium = truncated_normal_sample(
            row["K_MIN"],
            row["K_MAX"],
            SAMPLES_PER_CROP
        )
